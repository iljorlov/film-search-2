...
export const RecommendedFilms = () => {
  ...
  const recommendedFilms = useSelector((state: RootStore) => state.recommendedFilms.data)
    ...
    <>
      { recommendedFilms && recommendedFilms[pathFilmIndex] &&
        ...
        <FilmsList className='page'>
          <h2 id='recommended' ref={recommendedRef}>Recommended:</h2>
          {recommendedFilms![pathFilmIndex].total_pages > 0
            ? <Films>
                {recommendedFilms[pathFilmIndex].results.map((film:FilmType, index) => {
                  if( index === 20){
                    return ''
                  } else{
                    return (
        ...
        {recommendedFilms![pathFilmIndex].total_pages > 0 
          ? <RecommendedPagination top={top} currentPage={recommendedFilms[pathFilmIndex].page} totalPages={recommendedFilms[pathFilmIndex].total_pages} filtration={'film'} filmId={pathFilmIndex}/>
          : ''
        }
        </>
      }
